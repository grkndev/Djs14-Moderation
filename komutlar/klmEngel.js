    ...
    switch(SubCmd){
      case "durum":{
        ...
        if(durum === "aktif"){
          ...
          interaction.reply({ embeds:[new EmbedBuilder().setTitle("Kelime Engelleme Sistemi Aktif").setColor("Green").setDescription(`Kelime Engelleme Sistemi Yönetici Ttarafından aktif edildi artık istemediğiniz kelimeler engellenecek.`)] });
        }else if(durum === "pasif"){
          await model.updateOne({GuildID:guild.id},{kelimeEngl:false},{upsert:true});
          interaction.reply({ embeds:[new EmbedBuilder().setTitle("Kelime Engelleme Sistemi Pasif").setColor("Red").setDescription(`Kelime Engelleme Sistemi Yönetici Ttarafından devre dışı bırakıldı artık istemediğiniz kelimler engelenmeyecek.`)] });
        }
        break;
      }
