       ...
       // interaction.deferReply();
        switch(SubCmd){
            case "ayarla":{
                ...
                interaction.reply({embeds:[new EmbedBuilder().setTitle("Mod-Log Kanalı Ayarlandı!").setColor("Green").setDescription(`Mod-Log sistemi ayarlandı! Mod-Log kanalınız: <#${log_kanalı.id}>`)]});
                break;
            }
            case "sıfırla":{
                await model.updateOne({GuildID: interaction.guild.id},{modlogChannel: null},{upsert:true});
                interaction.reply({embeds:[new EmbedBuilder().setTitle("Mod-Log Kanalı Ayarlandı!").setColor("Red").setDescription(`Mod-Log sistemi kapatıldı! Artık sunucunuzda mod-log kanalı yok!`)]});
                break;
            }
        }
